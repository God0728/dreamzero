...
def main(
    ...
    if cfg_scale is not None:
        policy.trained_model.action_head.cfg_scale = float(cfg_scale)
        logger.info("Set action_head.cfg_scale=%s", policy.trained_model.action_head.cfg_scale)
    ...
    logger.info(
        "Starting WebsocketPolicyServer on %s:%d (DreamZero 5B, %dx%d, cfg_scale=%s)",
        host,
        port,
        h,
        w,
        policy.trained_model.action_head.cfg_scale,
    )
