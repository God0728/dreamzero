...
class WebsocketPolicyServer:
    ...
    async def _handler(self, websocket: websockets.asyncio.server.ServerConnection):
        ...
        logging.info("Policy server sending metadata to %s", websocket.remote_address)
        await websocket.send(packer.pack(dataclasses.asdict(self._server_config)))
        logging.info("Policy server metadata sent to %s", websocket.remote_address)
        ...
        while True:
            try:
                ...
                del obs["endpoint"]
                logging.info("Policy server handling endpoint=%s", endpoint)
                if endpoint == "reset":
                    reset_result = self._policy.reset(obs)
                    logging.info("Policy server completed reset")
                    to_return = packer.pack(reset_result) if reset_result is not None else "reset successful"
                else:
                    action = self._policy.infer(obs)
                    logging.info("Policy server completed infer")
