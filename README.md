...
以下流程默认面向 Unitree G1 硬件，用中文注释写成最短闭环，方便你快速打通新数据集到可训练的 DreamZero 微调流程：

1. 使用 `scripts/data/convert_lerobot_to_gear.py` 把 LeRobot v2 数据集转换成 GEAR 格式。
2. 检查 `meta/embodiment.json`、`meta/modality.json`、`meta/stats.json` 和 `meta/relative_stats_dreamzero.json` 是否都已生成，并且和你的 embodiment tag 一致。
3. 在 `groot/vla/configs/data/dreamzero/` 下新增数据集 YAML，同时在 `groot/vla/configs/data/dreamzero/base_48_wan_fine_aug_relative.yaml` 里注册对应的 modality / transform 配置。
4. 如果训练脚本需要 T5 cache，先预计算缓存；现成示例可以参考 `scripts/train/precompute_T5_cache.sh`、`scripts/train/precompute_T5_cache_unitree.sh` 和 `scripts/train/precompute_T5_cache_unitree_sweep_floor.sh`。
5. 直接运行现成的训练脚本，例如 `scripts/train/unitree_train_wan22_352x640_wb_v0-1.sh` 或 `scripts/train/unitree_sweep_floor_train_wan22_352x640_wb_v0-1.sh`。

更完整的新实体配置说明请看 [docs/DATASET_TO_GEAR_AND_TRAIN.md](docs/DATASET_TO_GEAR_AND_TRAIN.md)。如果你要继续适配别的机器人，可以先参考 YAM 或 Unitree 的脚本模板，只改数据路径、模态键、相机顺序和 relative-action key 列表。
