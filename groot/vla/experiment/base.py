...
def _run_git_snapshot_command(repo_root: Path, args) -> str:
...
def _write_training_git_diff(output_dir: Path) -> None:
    if os.environ.get("RANK", "0") not in {"", "0"}:
        return
    repo_root = Path(__file__).resolve().parents[3]
    output_path = output_dir / "git.diff"
    try:
        output_path.write_text(
            "\n".join(
                [
                    "# DreamZero git snapshot at training startup",
                    "",
                    "## Latest commit id and message",
                    "$ git log -1 --format=%H%n%B",
                    _run_git_snapshot_command(repo_root, ["log", "-1", "--format=%H%n%B"]).rstrip(),
                    "",
                    "## Git status",
                    "$ git status --untracked-files=all",
                    _run_git_snapshot_command(repo_root, ["status", "--untracked-files=all"]).rstrip(),
                    "",
                    "## Git diff relative to latest commit",
                    "$ git diff HEAD --",
                    _run_git_snapshot_command(repo_root, ["diff", "HEAD", "--"]).rstrip(),
                    "",
                ]
            ),
            encoding="utf-8",
        ...
        logging.exception("Failed to write training git snapshot to %s", output_path)
...
class BaseExperiment(ABC):
    def __init__(self, cfg: DictConfig):
        ...
        _write_training_git_diff(output_dir)
