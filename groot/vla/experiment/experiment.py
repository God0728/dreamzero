...
class VLATrainer(BaseTrainer):
    def __init__(self, **kwargs):
        ...
        self._rank_torch_rng_seeded = False

        super().__init__(**kwargs)

    def _seed_rank_specific_torch_rng_once(self):
        """Use PyTorch's global RNG streams, but offset them by DDP rank.

        Transformers resets all ranks to the same seed at the start of
        ``Trainer.train()``.  That is good for model initialization, but during
        training it makes fixed-shape diffusion batches draw identical noise on
        every rank.  Seed torch per rank after trainer setup so model weights
        stay synchronized while training-time torch randomness is rank-specific.
        """
        if self._rank_torch_rng_seeded:
            return
        seed = (
            int(self.args.seed)
            + int(self.rank)
            + int(self.state.global_step) * int(self.world_size)
        )
        torch.manual_seed(seed)
        if torch.cuda.is_available():
            torch.cuda.manual_seed_all(seed)
        self._rank_torch_rng_seeded = True
        if int(self.rank) == 0:
            logger.info("Seeded training torch RNG per rank with base seed %s", self.args.seed)

    def training_step(self, model, inputs, *args, **kwargs):
        self._seed_rank_specific_torch_rng_once()
