...
def _masked_mean_per_sample(
    ...
    """Average valid tokens inside each sample first, then average samples."""
    if mask is None:
        return values.flatten(1).mean(dim=1).mean()

    valid = mask.to(device=values.device, dtype=values.dtype)
    while valid.ndim < values.ndim:
        valid = valid.unsqueeze(-1)
    valid = torch.broadcast_to(valid, values.shape)

    per_sample_sum = (values * valid).flatten(1).sum(dim=1)
    per_sample_count = valid.flatten(1).sum(dim=1)
    valid_samples = per_sample_count > 0
    if not torch.any(valid_samples):
        return values.sum() * 0.0
    return (
        per_sample_sum[valid_samples]
        / per_sample_count[valid_samples].clamp_min(1.0)
    ).mean()
...
PromptCacheKey: TypeAlias = tuple[tuple[int, ...], bytes, tuple[int, ...], bytes]
...
class WANPolicyHeadConfig(PretrainedConfig):
    ...
    torch_compile_dit_blocks: bool = field(
        default=False,
        metadata={"help": "Compile Wan DiT block forwards with torch.compile."},
    )
    ...
    load_text_encoder: bool = field(
        default=True,
        metadata={"help": "Load and run the T5 text encoder. Disable only when training samples provide precomputed text_context."},
    )
...
class WANPolicyHead(ActionHead):
    ...
    def __init__(
        ...
        self.load_text_encoder = bool(getattr(config, "load_text_encoder", True))
        self.text_encoder = instantiate(config.text_encoder_cfg) if self.load_text_encoder else None
        self.image_encoder = instantiate(config.image_encoder_cfg)
        self.vae = instantiate(config.vae_cfg)
        self.scheduler = FlowMatchScheduler(shift=5, sigma_min=0.0, extra_one_step=True)
        self.model_names = ['text_encoder'] if self.text_encoder is not None else []
        ...
        self.language = None
        self._text_context_gpu_cache: dict[str, torch.Tensor] = {}
        self.enable_t5_prompt_cache = os.getenv("ENABLE_T5_PROMPT_CACHE", "true").lower() not in {
            "0",
            "false",
            "no",
            "off",
        }
        self._encoded_prompt_gpu_cache: dict[PromptCacheKey, torch.Tensor] = {}
        ...
        if hasattr(self.model, "_set_gradient_checkpointing"):
            self.model._set_gradient_checkpointing(self.model, self.use_gradient_checkpointing)
        elif hasattr(self.model, "gradient_checkpointing"):
            self.model.gradient_checkpointing = self.use_gradient_checkpointing
        elif hasattr(self.model, "use_gradient_checkpointing"):
            self.model.use_gradient_checkpointing = self.use_gradient_checkpointing
    ...
    def set_trainable_parameters(self, tune_projector: bool, tune_diffusion_model: bool):
        ...
        if not (self.train_architecture == "lora" and self.defer_lora_injection):
            self._maybe_torch_compile_dit_blocks(reason="trainable-parameter setup")
    ...
    def _get_dit_blocks(self):
        """Return Wan DiT blocks, including PEFT-wrapped models."""
        for candidate in (
            self.model,
            ...
            blocks = getattr(candidate, "blocks", None) if candidate is not None else None
            if blocks is not None:
                return blocks
        return None

    def _maybe_torch_compile_dit_blocks(self, reason: str) -> None:
        if not bool(getattr(self.config, "torch_compile_dit_blocks", False)):
            return
        if os.getenv("ENABLE_TENSORRT", "False").lower() == "true":
            self._rank0_print("Skipping DiT block torch.compile because ENABLE_TENSORRT=true.")
            return

        blocks = self._get_dit_blocks()
        if blocks is None:
            self._rank0_print("Requested DiT block torch.compile, but no model.blocks were found.")
            return

        compile_options = {}
        try:
            compile_options.update(torch._inductor.list_mode_options("reduce-overhead"))
        except Exception:
            pass
        # CUDA Graph capture is intentionally off for repeated AR diffusion calls;
        # block-level Inductor compile keeps the stable speedup without replay hazards.
        ...
        for block in blocks:
            total_count += 1
            if getattr(block, "_dz_torch_compile_dit_block_forward", False):
                continue
            ...
            block._dz_torch_compile_dit_block_forward = True
            compiled_count += 1

        if compiled_count:
            self._rank0_print(
                "Torch compiled "
                f"{compiled_count}/{total_count} DiT blocks for {reason} "
                "(mode=reduce-overhead, fullgraph=True, dynamic=True, cudagraphs=False)."
            )
    ...
    def inject_lora_after_loading(self):
        ...
        if self.train_architecture == "lora":
            ...
            self.vae.requires_grad_(False)
            self.print_trainable_params()
            self._maybe_torch_compile_dit_blocks(reason="deferred LoRA injection")
        else:
    ...
    def encode_prompt(self, input_ids, attention_mask):
        if self.text_encoder is None:
            raise ValueError(
                "T5 text_encoder is not loaded. Provide precomputed text_context/text_context_mask "
                "or set action_head_cfg.config.load_text_encoder=true."
            )
    ...
    @staticmethod
    def _prompt_cache_key(input_ids: torch.Tensor, attention_mask: torch.Tensor) -> PromptCacheKey:
        ids = input_ids.detach().to(device="cpu", dtype=torch.int64).contiguous()
        mask = attention_mask.detach().to(device="cpu", dtype=torch.bool).contiguous()
        return tuple(ids.shape), ids.numpy().tobytes(), tuple(mask.shape), mask.numpy().tobytes()

    def encode_prompt_cached(self, input_ids: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
        if not self.enable_t5_prompt_cache or self.training or torch.is_grad_enabled():
            return self.encode_prompt(input_ids, attention_mask).to(self._device)
        cache_key = self._prompt_cache_key(input_ids, attention_mask)
        prompt_emb = self._encoded_prompt_gpu_cache.get(cache_key)
        if prompt_emb is None:
            prompt_emb = self.encode_prompt(input_ids, attention_mask).to(self._device).detach()
            prompt_emb.requires_grad_(False)
            self._encoded_prompt_gpu_cache[cache_key] = prompt_emb
        return prompt_emb
    ...
    def _get_prompt_embeddings(
        ...
        data: BatchFeature,
        *,
        context_key: str = "text_context",
        cache_path_key: str = "text_context_cache_path",
        text_key: str = "text",
        mask_key: str = "text_attention_mask",
    ) -> torch.Tensor:
        if context_key in data and data[context_key] is not None:
            prompt_emb = data[context_key]
            if prompt_emb.ndim == 2:
                prompt_emb = prompt_emb.unsqueeze(0)
            return prompt_emb.to(device=self._device, dtype=torch.bfloat16)
        if cache_path_key in data and data[cache_path_key] is not None:
            return self._get_prompt_embeddings_from_cache_paths(data[cache_path_key])
        return self.encode_prompt_cached(data[text_key], data[mask_key])

    def _get_prompt_embeddings_from_cache_paths(self, cache_paths) -> torch.Tensor:
        if isinstance(cache_paths, (str, os.PathLike)):
            cache_paths = [cache_paths]
        cache_paths = [os.fspath(path) for path in cache_paths]

        prompt_embs: list[torch.Tensor] = []
        for cache_path in cache_paths:
            cached_context = self._text_context_gpu_cache.get(cache_path)
            if cached_context is None:
                ...
                if context.ndim != 2:
                    raise ValueError(
                        f"Cached text context must be [L, D], got {tuple(context.shape)} in {cache_path}"
                    )
                context = context.to(device=self._device, dtype=torch.bfloat16)
                if context_mask is not None:
                    context_mask = context_mask.to(device=self._device, dtype=torch.bool)
                    if context_mask.ndim != 1 or context_mask.shape[0] != context.shape[0]:
                        raise ValueError(
                            f"Cached text mask must be [L] matching context, got "
                            f"{tuple(context_mask.shape)} for {tuple(context.shape)} in {cache_path}"
                        )
                    context = context.clone()
                    context[~context_mask] = 0
                cached_context = context.detach()
                cached_context.requires_grad_(False)
            self._text_context_gpu_cache[cache_path] = cached_context
            prompt_embs.append(cached_context)

        if len(prompt_embs) == 1:
            return prompt_embs[0].unsqueeze(0)
        return torch.stack(prompt_embs, dim=0)
    ...
    def encode_image_conditioning(self, image, num_frames, height, width):
        """Encode first-frame conditioning for the Wan backbone.

        Wan2.1-style I2V checkpoints concatenate the first-frame VAE latent
        (`y`) with the noisy latent before patch embedding, so training must
        build the masked latent condition. Wan2.2 TI2V uses CLIP first-frame
        conditioning only (`concat_first_frame_latent=false`), so computing
        `y` would run an extra frozen VAE encode that the DiT never consumes.
        Keep the original `y` path for non-TI2V variants because the I2V train
        path still asserts that `y` is present.
        """
        if (
            getattr(self.model, "model_type", None) == "ti2v"
            and not getattr(self.model, "concat_first_frame_latent", True)
        ):
            with torch.amp.autocast(dtype=torch.bfloat16, device_type=torch.device(self._device).type):
                clip_context = self.image_encoder.encode_image(image)
            return clip_context, None

        clip_context, y, _ = self.encode_image(image, num_frames, height, width)
    ...
    def forward(self, backbone_output: BatchFeature, action_input: BatchFeature) -> BatchFeature:
        ...
        latents = self.encode_video(videos, self.tiled, (self.tile_size_height, self.tile_size_width), (self.tile_stride_height, self.tile_stride_width))  # (False, (34, 34), (18, 16))

        # print("latents shape", latents.shape, self.dtype)
        _, _, num_frames, height, width = videos.shape
        image = videos[:, :, :1].transpose(1, 2)  # 第一帧

        clip_feas, ys = self.encode_image_conditioning(image, num_frames, height, width)
        ...
        video_temporal_mask = data.get("video_temporal_mask", None)
        latent_video_temporal_mask = None
        if video_temporal_mask is not None:
            video_temporal_mask = video_temporal_mask.bool().to(self._device)
            latent_video_len = noise.shape[1]
            if video_temporal_mask.shape[1] == latent_video_len:
                latent_video_temporal_mask = video_temporal_mask
            else:
                input_video_len = video_temporal_mask.shape[1]
                if latent_video_len == 1:
                    temporal_indices = torch.zeros(
                        (1,), dtype=torch.long, device=video_temporal_mask.device
                    )
                elif (input_video_len - 1) % (latent_video_len - 1) == 0:
                    temporal_stride = (input_video_len - 1) // (latent_video_len - 1)
                    temporal_indices = (
                        torch.arange(
                            latent_video_len,
                            dtype=torch.long,
                            device=video_temporal_mask.device,
                        )
                        * temporal_stride
                    )
                else:
                ...
                latent_video_temporal_mask = video_temporal_mask.index_select(
                    1, temporal_indices
                )
        ...
        with torch.amp.autocast(dtype=torch.bfloat16, device_type=torch.device(self._device).type):
            ...
            dynamics_weight = self.scheduler.training_weight(
                timestep.flatten(0, 1)
            ).unflatten(0, (noise.shape[0], noise.shape[1])).to(self._device)
            weight_dynamics = dynamics_loss_per_sample * dynamics_weight
            weighted_dynamics_loss = _masked_mean_per_sample(
                weight_dynamics,
                latent_video_temporal_mask,
            )
            ...
            if actions.numel() > 0:
                ...
                weight_action = action_loss_per_sample * action_weight[:, :, None]
                action_loss_mask = action_mask.bool()
                if "action_temporal_mask" in data:
                    action_loss_mask = action_loss_mask & data[
                        "action_temporal_mask"
                    ].bool().to(action_loss_mask.device)[:, :, None]
                weighted_action_loss = _masked_mean_per_sample(
                    weight_action,
                    action_loss_mask,
                )
    ...
    def lazy_joint_video_action(self, backbone_output: BatchFeature, action_input: BatchFeature, latent_video: torch.Tensor | None = None) -> BatchFeature:
        ...
        if self.ip_size == 1 and self.cfg_scale == 1.0:
            prompt_embs = [self._get_prompt_embeddings(data)]
        else:
            text_inputs = self._prepare_text_inputs(data)
            prompt_embs = [self.encode_prompt_cached(text, attention_mask) for text, attention_mask in text_inputs]
        ...
        # noise_obs size(1 3 2 22 40)  noise_action(1 48 53)
        ...
        tokens_per_frame = (height // 2) * (width // 2) # 11*20 = 220
        frame_seqlen = tokens_per_frame
        seq_len = num_frames * frame_seqlen # 2 * 220  
        ...
        assert self.kv_cache1 is not None  # len 30  torch.Size([2, 1, 0, 24, 128])
        assert self.kv_cache_neg is not None  # len 30
        assert self.crossattn_cache is not None
        assert self.crossattn_cache_neg is not None   # torch.Size([2, 1, 512, 24, 128])
        ...
        for index, current_timestep in enumerate(sample_scheduler.timesteps):
            ...
            if should_run_model:
                ...
                if self.current_start_frame + self.num_frame_per_block <= self.ys.shape[2]:
                    y = self.ys[:, :, self.current_start_frame : self.current_start_frame + self.num_frame_per_block] # torch.Size([1, 52, 2, 22, 40])  t= 1 t = 2
                else:
                    y = self.ys[:, :, -self.num_frame_per_block:]
                ...
                flow_pred_cond, flow_pred_cond_action = predictions[0]  # flow_pred_cond  torch.Size([1, 48, 2, 22, 40])
                if len(predictions) == 1:
                    # No-CFG single-GPU path: _prepare_text_inputs returns only
                    # the conditional prompt when cfg_scale == 1.0, so avoid
                    # the negative-prompt DiT pass and use the conditional video
                    # prediction directly.  Keep this guarded so an accidental
                    # missing negative branch cannot silently disable video CFG.
                    if not (self.ip_size == 1 and float(self.cfg_scale) == 1.0):
                        raise RuntimeError(
                            "single prediction branch is only valid for no-CFG single-GPU inference "
                            f"(ip_size=1, cfg_scale=1.0); got ip_size={self.ip_size}, "
                            f"cfg_scale={self.cfg_scale}, len(predictions)={len(predictions)}"
                        )
                    ...
                    raise RuntimeError(f"expected 1 or 2 prediction branches, got {len(predictions)}")
    ...
    def post_initialize(self):
        ...
        # Torch compile the frozen inference modules. DiT block compile is controlled
        # separately by torch_compile_dit_blocks and is safe for both train/inference.
        if not ENABLE_TENSORRT:
            print(
                "Torch compiling the TextEncoder, ImageEncoder, and VAE modules "
                "(Wan DiT blocks are configured separately)."
            )
            ...
            self._maybe_torch_compile_dit_blocks(reason="post-initialize")
