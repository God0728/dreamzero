...
class GrootSimPolicy(BaseGrootSimPolicy):
    def __init__(
        ...
        if not exp_cfg_dir.exists() and model_dir.parent.joinpath("experiment_cfg").exists():
            exp_cfg_dir = model_dir.parent / "experiment_cfg"
        ...
        # Keep metadata.json as the raw input contract for early transforms such as
        # VideoToTensor.  Model/action-head target_video_height/width is a downstream
        # resize target; applying it to metadata here makes VideoToTensor reject valid
        # raw observations before VideoResize can run (e.g. LIBERO 128x128 -> 320x160).
        if skip_img_transform:
            for t in eval_transform_cfg.transforms:
                if t._target_ == "groot.vla.data.transform.VideoCrop":
                    ...
                    for key in metadata.modalities.video.keys():
                        # DatasetMetadata stores video resolution as
                        # (width, height), while VideoResize config stores
                        # (height, width).  VideoToTensor validates HWC input
                        # by reversing data.shape[-3:-1] to (W, H), so keep
                        # metadata in the DatasetMetadata convention when
                        # client-side resize replaces the skipped resize
                        # transform.
                        metadata.modalities.video[key].resolution = (width, height)
