...
Usage:
  ...
  # Datasets with separate LeRobot columns per field:
  python scripts/data/convert_lerobot_to_gear.py \\
      --dataset-path ./Dataset/unitree_task \\
      --embodiment-tag unitree_g1_upper_body \\
      --state-keys '{"left_gripper": "observation.state.left_gripper"}' \\
      --action-keys '{"left_gripper": "action.left_gripper"}'
...
VALID_EMBODIMENT_TAGS = [
    ...
    "dream", "yam", "xdof", "libero_sim",
    "unitree_g1_upper_body",
...
def _feature_dim(features: dict, original_key: str) -> int:
    """Infer a feature dimension from meta/info.json."""
    if original_key not in features:
        log.error("Key '%s' not found in meta/info.json features", original_key)
        sys.exit(1)
    shape = features[original_key].get("shape", [1])
    if shape in (None, []):
        return 1
...
def _normalize_key_spec(name: str, spec: Any) -> dict[str, Any]:
    """Normalize one state/action mapping entry.

    Supported forms:
      - "name": [start, end]                      # legacy, uses default column
      - "name": "original.column"                 # full separate column
      - "name": {"original_key": "...", "start": 0, "end": 6}
      - "name": {"key": "...", "range": [0, 6]}
    """
    if isinstance(spec, list):
        if len(spec) != 2:
            log.error("Each list mapping must be [start, end]. Got %s for '%s'", spec, name)
            sys.exit(1)
        return {"original_key": None, "start": int(spec[0]), "end": int(spec[1])}
    if isinstance(spec, str):
        return {"original_key": spec, "start": None, "end": None}
    if isinstance(spec, dict):
        original_key = spec.get("original_key", spec.get("key", spec.get("column")))
        bounds = spec.get("range", spec.get("slice"))
        start = spec.get("start")
        end = spec.get("end")
        if bounds is not None:
            if not isinstance(bounds, list) or len(bounds) != 2:
                log.error("'range'/'slice' must be [start, end]. Got %s for '%s'", bounds, name)
                sys.exit(1)
            start, end = bounds
        return {
    ...
    log.error(
        "Invalid mapping for '%s'. Expected [start,end], original-key string, or object; got %r",
        name,
        spec,
    )
    sys.exit(1)
...
def parse_key_mapping(raw: str | None) -> dict[str, dict[str, Any]] | None:
    """Parse state/action mapping JSON.

    Backward-compatible legacy syntax is still accepted:
      '{"joint_pos": [0, 6], "gripper": [6, 7]}'

    For separate-column LeRobot datasets, use:
    ...
    if not isinstance(mapping, dict):
        log.error("Key mapping must be a JSON object, got %r", mapping)
        sys.exit(1)
    return {name: _normalize_key_spec(name, spec) for name, spec in mapping.items()}
...
def build_modality_json(
    info: dict,
    detected: dict,
    state_mapping: dict[str, dict[str, Any]] | None,
    action_mapping: dict[str, dict[str, Any]] | None,
    task_key: str | None,
) -> dict:
    ...
    if state_col and state_mapping:
        for name, spec in state_mapping.items():
            original_key = spec.get("original_key") or state_col
            start = spec.get("start")
            end = spec.get("end")
            if start is None:
                start = 0
            if end is None:
                end = _feature_dim(features, original_key)
    ...
    if action_col and action_mapping:
        for name, spec in action_mapping.items():
            original_key = spec.get("original_key") or action_col
            start = spec.get("start")
            end = spec.get("end")
            if start is None:
                start = 0
...
def compute_relative_stats(
    ...
    for rel_key in relative_action_keys:
        ...
        for pp in tqdm(parquet_paths, desc=f"Relative stats [{rel_key}]"):
            ...
            traj_len = len(df)
            # Match LeRobotSingleDataset._calculate_relative_stats_for_key:
            # for an action horizon of H, offsets are [0, ..., H-1], so the
            # last valid anchor is traj_len - H.
            usable = traj_len - (action_horizon - 1)
            for i in range(max(usable, 0)):
                ref_state = state_slice[i]
                chunk_end = i + action_horizon
...
def main():
    ...
    parser.add_argument(
        ...
        help='JSON mapping of state sub-keys to [start, end] ranges or source columns, '
             'e.g. \'{"joint_pos": [0, 6], "left_gripper": "observation.state.left_gripper"}\''
    )
    parser.add_argument(
        "--action-keys", type=str, default=None,
        help='JSON mapping of action sub-keys to [start, end] ranges or source columns'
    )
    parser.add_argument(
