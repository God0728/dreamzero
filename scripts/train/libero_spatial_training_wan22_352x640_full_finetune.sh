#!/bin/bash
# LIBERO-Spatial Wan2.2-TI2V-5B from-Wan training, full DiT fine-tune.
# Final model/VAE input is the side-by-side composite HxW=352x640 (frame_seqlen=220).
...
OUTPUT_DIR=${OUTPUT_DIR:-"/DATA/disk2/yuhangwang/data/WorldModel/dreamzero/trains/libero_spatial_wan22_352x640_full_from_wan"}
...
# Per-view 320x352 becomes a 640x352 side-by-side composite in DreamTransform;
# target_video_height/width matches that composite, so the action head should not
# rescale before VAE encode.
torchrun --nproc_per_node "$NUM_GPUS" --standalone groot/vla/experiment/experiment.py \
  ...
  num_views=2 \
  model=dreamzero/vla \
  model/dreamzero/action_head=wan_flow_matching_action_tf_wan22_libero_352x640_full \
