#!/bin/bash
# LIBERO all-subsets Wan2.2-TI2V-5B from-Wan training, full DiT fine-tune.
...
LIBERO_ALL_DATA_ROOT=${LIBERO_ALL_DATA_ROOT:-"/DATA/disk2/yuhangwang/data/WorldModel/libero_mujoco3.3.2"}
OUTPUT_DIR=${OUTPUT_DIR:-"/DATA/disk2/yuhangwang/data/WorldModel/dreamzero/trains/libero_wan22_160x320_full_gbs8_v0.0"}
...
if [ ! -d "$LIBERO_ALL_DATA_ROOT" ]; then
  echo "ERROR: LIBERO all-subsets parent dataset not found at $LIBERO_ALL_DATA_ROOT"
  exit 1
fi
for subset in \
...
do
  if [ ! -f "$LIBERO_ALL_DATA_ROOT/$subset/meta/embodiment.json" ]; then
    echo "ERROR: $LIBERO_ALL_DATA_ROOT/$subset/meta/embodiment.json is missing"
    exit 1
  fi
done
...
torchrun --nproc_per_node "$NUM_GPUS" --standalone groot/vla/experiment/experiment.py \
  ...
  max_chunk_size=4 \
  save_strategy=steps \
  libero_all_data_root="$LIBERO_ALL_DATA_ROOT" \
  ...
  # +train_dataset.dataset_kwargs.num_steps_per_shard=1000
