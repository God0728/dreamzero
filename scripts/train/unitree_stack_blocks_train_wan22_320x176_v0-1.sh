#!/bin/bash
# Unitree stack-blocks Wan2.2-TI2V-5B 全量 DiT 微调脚本。
# 数据需先用 scripts/data/convert_stack_blocks_to_lerobot_gear.py 转换。
# state/action 为单一 60-D key:robot_q[:36] + hand[:12] + ee_state[:12]。
# 60-D action 以相对动作(relative action)形式针对当前 state 训练。
# 硬件与 sweep_floor 完全一致,直接复用 unitree_g1_upper_body embodiment。
...
STACK_BLOCKS_DATA_ROOT=${STACK_BLOCKS_DATA_ROOT:-"/mnt/raid0/unitree_brainco/gear_format/stack_blocks_lerobot_gear_50eps"}
OUTPUT_DIR=${OUTPUT_DIR:-"/mnt/raid0/dreamzero_checkpoints/unitree_stack_blocks_wan22_320x176_v0.1_0616_50eps"}
T5_CACHE_DIR=${T5_CACHE_DIR:-"/mnt/raid0/dreamzero_models/t5_cache/unitree_stack_blocks_50eps"}
...
if [ ! -d "$STACK_BLOCKS_DATA_ROOT" ]; then
  echo "ERROR: converted stack-blocks dataset not found at $STACK_BLOCKS_DATA_ROOT"
  echo "Run: python3 scripts/data/convert_stack_blocks_to_lerobot_gear.py --force"
  exit 1
fi
for meta_file in info.json modality.json embodiment.json stats.json relative_stats_dreamzero.json tasks.jsonl episodes.jsonl; do
  if [ ! -f "$STACK_BLOCKS_DATA_ROOT/meta/$meta_file" ]; then
    echo "ERROR: $STACK_BLOCKS_DATA_ROOT/meta/$meta_file is missing."
    echo "Run: python3 scripts/data/convert_stack_blocks_to_lerobot_gear.py --force"
    exit 1
  fi
done
"$PYTHON_EXEC" - "$STACK_BLOCKS_DATA_ROOT" <<'PY'
import json
import sys
...
for key in ("observation.state.sweep_floor_control", "action.sweep_floor_control"):
    ...
    if shape != [expected_dim]:
        print(
            ...
            "Re-run scripts/data/convert_stack_blocks_to_lerobot_gear.py --force "
...
if [ ! -d "$T5_CACHE_DIR" ] || [ -z "$(ls -A "$T5_CACHE_DIR" 2>/dev/null)" ]; then
  ...
  echo "Run: bash scripts/train/precompute_T5_cache_unitree_stack_blocks.sh"
  exit 1
fi
...
"$PYTHON_EXEC" -m torch.distributed.run \
  ...
  report_to="$REPORT_TO" \
  data=dreamzero/unitree_stack_blocks_relative_wan22 \
  ...
  stack_blocks_data_root="$STACK_BLOCKS_DATA_ROOT" \
