#!/usr/bin/env python3
"""Episode eval for Unitree sweep-floor DreamZero checkpoints.

This script evaluates a sweep-floor checkpoint on complete LeRobot/GEAR
...
def _extract_sweep_action(act: Any, *, horizon: int) -> np.ndarray:
    ...
    if arr.ndim != 2 or arr.shape[-1] < ACTION_DIM:
        raise ValueError(f"Expected sweep action [T,>=53], got {arr.shape}")
    arr = arr[:, :ACTION_DIM]
...
def _action_dim_names() -> list[str]:
    names = [f"robot_q_desired[{i}]" for i in range(29)]
    names += [f"hand_cmd[{i}]" for i in range(12)]
    names += [f"ee_state[{i}]" for i in range(12)]
    return names
...
def evaluate(args: argparse.Namespace) -> None:
    ...
    for episode_id in episode_ids:
        ...
        for chunk_idx in range(num_chunks):
            ...
            pred_chunks.append(_extract_sweep_action(result_batch.act, horizon=args.action_horizon))
        ...
        save_action_plot(
            ...
            pred=pred_action,
            gt=gt_action,
            title=f"sweep-floor episode {episode_id} action | MSE={action_mse:.5g}",
        )
        ...
        metrics = {"action_mse": action_mse, "action_mae": action_mae, **_video_metrics(pred_video, gt_video)}
        summary = {
